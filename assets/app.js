/* 医学应试复习系统 — 核心逻辑（纯原生 JS，零构建，兼容 file:// 与 GitHub Pages） */
(function () {
  "use strict";
  ...
  /* ---------- 数据加载与合并 ---------- */
  function loadData() {
    let saved = store.get(DKEY);
    if (saved) { try { return JSON.parse(saved); } catch (e) {} }
    return window.SUBJECTS;
  }
  let DATA = loadData();

  function saveData() { store.set(DKEY, JSON.stringify(DATA)); }

  function loadState() {
    let s = store.get(SKEY);
    if (s) { try { return JSON.parse(s); } catch (e) {} }
    return {};
  }
  let STATE = loadState();
  function saveState() { store.set(SKEY, JSON.stringify(STATE)); }

  function key(sub, pid) { return sub + ":" + pid; }
  function getSt(sub, pid) { return STATE[key(sub, pid)] || {}; }
  function updSt(sub, pid, patch) {
    const k = key(sub, pid);
  ...
  function subjects() { return DATA; }
  function curSubject() { return subjects()[App.subject]; }
  function curChapter() { return curSubject().chapters.find(c => c.id === App.chapterId); }
  function curPoint() {
    const ch = curChapter();
    if (!ch || !ch.points) return null;
    return ch.points.find(p => p.id === App.pointId);
  }
  ...
  function renderTree() {
    ...
    document.getElementById("sideTitle").textContent = curSubject().name;
  }
  ...
  function pointValue(p, d, sub, pid) {
    // 状态维度优先取复习状态
    if (d.state) {
      const st = getSt(sub, pid);
      switch (d.k) {
        case "c4_forget": return st.forget || (p.dims && p.dims.c4_forget) || "未评估";
        case "c4_lastreview": return st.lastReview ? fmtDate(st.lastReview) : "未复习";
        case "c4_nextreview": return st.nextReview ? fmtDate(st.nextReview) + (days(st.nextReview) <= 0 ? "（已到期）" : `（${days(st.nextReview)}天后）`) : "—";
        case "c5_mastery": return st.mastery || (p.dims && p.dims.c5_mastery) || "未评估";
        case "c5_weak": return st.weak ? "是" : ((p.dims && p.dims.c5_weak) || "否");
        case "c5_weaksource": return st.weaksource || (p.dims && p.dims.c5_weaksource) || "";
        case "c5_errfreq": return st.errfreq || (p.dims && p.dims.c5_errfreq) || "";
        case "c5_impact": return st.impact || (p.dims && p.dims.c5_impact) || "";
        case "c5_priority": return st.priority || (p.dims && p.dims.c5_priority) || "";
      }
    }
    return (p.dims && p.dims[d.k]) || "";
  }
  ...
  function buildQueue() {
    ...
    let pts = [];
    sub.chapters.forEach(ch => { if (ch.points) ch.points.forEach(p => pts.push({ ch: ch.name, p: p })); });
    if (App.reviewFilter === "due") {
      pts = pts.filter(o => { const st = getSt(App.subject, o.p.id); return !st.lastReview || (st.nextReview && st.nextReview <= Date.now()); });
  ...
  function renderReview() {
    ...
    if (App.reviewIdx >= App.reviewQueue.length) {
      ...
      const r = document.getElementById("rvRestart");
      if (r) r.onclick = () => { App.reviewIdx = 0; App._revealed = false; buildQueue(); renderReview(); };
      return;
    }
    const item = App.reviewQueue[App.reviewIdx];
    const p = item.p;
    const revealed = App._revealed;
    let recallList = DIMENSIONS.filter(d => !d.state).map(d => d.t).join("、");
    ...
    c.querySelectorAll(".chips button").forEach(b => b.onclick = () => { App.reviewFilter = b.dataset.f; buildQueue(); App._revealed = false; renderReview(); });
    const sa = document.getElementById("showAns");
    if (sa) sa.onclick = () => { App._revealed = true; renderReview(); };
    c.querySelectorAll(".rating button[data-g]").forEach(b => b.onclick = () => rate(b.dataset.g));
  }

  function rate(grade) {
    const item = App.reviewQueue[App.reviewIdx];
    const pid = item.p.id;
    const st = Object.assign({}, getSt(App.subject, pid));
    st.reps = st.reps || 0;
    st.interval = st.interval || 0;
    const now = Date.now();
    st.lastReview = now;
    if (grade === "again") { st.reps = 0; st.interval = 0; st.mastery = "低"; st.weak = true; }
    else if (grade === "hard") { st.reps += 1; st.interval = Math.max(1, Math.round(st.interval * 1.3) || 1); st.mastery = st.reps >= 2 ? "高" : "中"; }
    else if (grade === "good") { st.reps += 1; st.interval = Math.max(2, Math.round(st.interval * 2.5) || 2); st.mastery = st.reps >= 2 ? "高" : "中"; if (st.reps >= 3) st.weak = false; }
    else if (grade === "easy") { st.reps += 1; st.interval = Math.max(4, Math.round(st.interval * 4) || 4); st.mastery = "高"; st.weak = false; }
    st.nextReview = now + st.interval * 86400000;
  ...
  function renderEdit() {
    ...
    if (!p) {
      html += `<div class="card"><div class="empty-tip">请选择或新增一个知识点进行编辑。</div></div>`;
      c.innerHTML = html; bindEdit(); return;
    }
    p.dims = p.dims || {};
    ...
    GROUPS.forEach(g => {
      ...
      DIMENSIONS.filter(d => d.g === g.id).forEach(d => {
        ...
        if (isSel) {
          ...
          opts.forEach(op => { o += `<option value="${op}" ${String(val) === op ? "selected" : ""}>${op || "（空）"}</option>`; });
          o += `</select>`;
  ...
  function bindEdit() {
    const save = () => {
      const p = curPoint(); if (!p) return;
      p.title = document.getElementById("f_title").value;
      p.dims = p.dims || {};
      DIMENSIONS.forEach(d => {
        if (d.k === "c4_lastreview" || d.k === "c4_nextreview") return;
        const el = document.getElementById("f_" + d.k);
        if (!el) return;
        if (d.state) {
          const st = Object.assign({}, getSt(App.subject, p.id));
    ...
    const addPoint = () => {
      const ch = curChapter(); ch.points = ch.points || [];
      const pid = "u" + Date.now();
      ch.points.push({ id: pid, title: "新知识点", dims: {} });
      App.pointId = pid; saveData(); renderTree(); renderEdit();
    };
    const addCh = () => {
      const sub = curSubject();
      const cid = "c" + Date.now();
      sub.chapters.push({ id: cid, name: "新章节", points: [] });
      App.chapterId = cid; App.pointId = null; saveData(); renderTree(); renderEdit();
    };
    const delPoint = () => {
      const ch = curChapter(); if (!ch.points) return;
      const i = ch.points.findIndex(p => p.id === App.pointId);
      if (i < 0) return;
      if (!confirm("确认删除该知识点？")) return;
      ch.points.splice(i, 1); App.pointId = ch.points[0] ? ch.points[0].id : null;
      saveData(); renderTree(); renderEdit();
    };
    ["edSave", "edSave2"].forEach(id => { const e = document.getElementById(id); if (e) e.onclick = save; });
    ["edExport", "edExport2"].forEach(id => { const e = document.getElementById(id); if (e) e.onclick = exp; });
    const ap = document.getElementById("edAddPoint"); if (ap) ap.onclick = addPoint;
    const ac = document.getElementById("edAddCh"); if (ac) ac.onclick = addCh;
    const dp = document.getElementById("edDelPoint"); if (dp) dp.onclick = delPoint;
  }

  function exportSubject() {
    const sub = curSubject();
    const txt = "window.SUBJECTS = window.SUBJECTS || {};\nwindow.SUBJECTS." + sub.id + " = " + JSON.stringify(sub, null, 2) + ";\n";
  ...
  function updateStat() {
    const sub = curSubject();
    let due = 0, weak = 0, total = 0;
    sub.chapters.forEach(ch => { if (ch.points) ch.points.forEach(p => {
      total++; const st = getSt(App.subject, p.id);
      if (!st.lastReview || (st.nextReview && st.nextReview <= Date.now())) due++;
      if (st.weak) weak++;
    }); });
    document.getElementById("statBadge").textContent = `待复习 ${due} · 薄弱 ${weak} / 共 ${total}`;
  }

  /* ---------- 模式切换 ---------- */
  function setMode(m) {
    if (m === "edit" && window.innerWidth <= 768) { alert("编辑模式仅在电脑端可用，手机端仅做复习。"); return; }
    App.mode = m; App._revealed = false;
    if (m === "review") { App.reviewFilter = App.reviewFilter || "due"; buildQueue(); }
    document.querySelectorAll(".mode").forEach(b => b.classList.toggle("active", b.dataset.mode === m));
    document.querySelectorAll(".mobilebar button[data-mode]").forEach(b => b.classList.toggle("active", b.dataset.mode === m));
    renderMode();
  }
  function renderMode() {
  ...
  /* ---------- 启动 ---------- */
  function init() {
    if (!window.SUBJECTS || !Object.keys(window.SUBJECTS).length) {
      document.getElementById("content").innerHTML = `<div class="empty-tip">未加载到数据文件，请确认 data/*.js 存在。</div>`;
      return;
    }
